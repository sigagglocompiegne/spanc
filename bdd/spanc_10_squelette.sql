...
INSERT INTO m_spanc.lt_spanc_typimmeuble(
    ...
    ('20','Collectif'),
    ('31','Local commercial'),
    ('32','Autres locaux (hors habitation et hors ICPE)'),
...
COMMENT ON TABLE m_spanc.lt_spanc_contdt
    IS 'Liste de valeurs sur les conclusions de la demande de travaux';

COMMENT ON COLUMN m_spanc.lt_spanc_contdt.code
    IS 'Code des types sur les conclusions de la demande de travaux';

COMMENT ON COLUMN m_spanc.lt_spanc_contdt.valeur
    IS 'Valeur des types sur les conclusions de la demande de travaux';
...
begin
 ...
 -- ##### FIN DES ERREURS LIEES AUX LISTES DES EQUIPEMENTS #######
...
COMMENT ON COLUMN m_spanc.an_spanc_contact.date_maj
    IS 'Date de mise à jour des informations d''installation';
COMMENT ON COLUMN m_spanc.an_spanc_contact.observ
...
(
    ...
    equ_ventil character varying(1) COLLATE pg_catalog."default",
    contr_concl character varying(2) COLLATE pg_catalog."default" DEFAULT '00'::character varying not NULL,
    contr_concl_a character varying(2) COLLATE pg_catalog."default",
    ...
    contr_nreal character varying(2) COLLATE pg_catalog."default" DEFAULT 'ZZ'::character varying not NULL,
    date_dem timestamp without time zone NOT NULL,
    ...
    CONSTRAINT an_spanc_controle_contrconcl_fkey FOREIGN KEY (contr_concl)
        ...
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
    CONSTRAINT an_spanc_controle_demconcl_fkey FOREIGN KEY (dem_concl)
        REFERENCES m_spanc.lt_spanc_contdt (code) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION,
...
COMMENT ON COLUMN m_spanc.an_spanc_controle.date_env
    IS 'Date d''envoi de la demande';    
...
COMMENT ON COLUMN m_spanc.an_spanc_controle.equ_ventil
...
COMMENT ON COLUMN m_spanc.an_spanc_controle.contr_concl_a
    IS 'Conclusion des anciens contrôles (P1,P2 P3)';   
...
CREATE TABLE m_spanc.an_spanc_conf (
	...
	contr_abs smallint,
	rel_perio smallint,
	rel_vente_nc smallint,
	rel_vente_ai smallint,
	date_sai timestamp without time ZONE NOT NULL,
    date_maj timestamp without time zone,
    op_sai character varying(20) COLLATE pg_catalog."default" NOT NULL,
    op_maj character varying(20) COLLATE pg_catalog."default",
...
COMMENT ON COLUMN m_spanc.an_spanc_conf.rel_vente_nc
    IS 'Délais de relance lié à un contrôle de vente avec une non conformité (grave ou pas)';
COMMENT ON COLUMN m_spanc.an_spanc_conf.rel_vente_ai
    IS 'Délais de relance lié à un contrôle de vente avec une absence d''installation';   
...
truncate table m_spanc.an_spanc_conf;   
insert into m_spanc.an_spanc_conf 
values
(nextval('m_spanc.an_spanc_conf_seq'::regclass),10,10,4,6,6,12,6,now(),null,'lcclo',null,'cclo'),
(nextval('m_spanc.an_spanc_conf_seq'::regclass),8,4,4,12,0,12,12,now(),null,'lccpe',null,'ccpe'),
(nextval('m_spanc.an_spanc_conf_seq'::regclass),10,10,4,6,6,12,12,now(),null,'larc',null,'arc'),
(nextval('m_spanc.an_spanc_conf_seq'::regclass),8,4,4,12,6,12,12,now(),null,'lcc2v',null,'cc2v');
